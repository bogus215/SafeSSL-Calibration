...
class TopKMixupAccuracy(nn.Module):
    ...
    @torch.no_grad()
    def forward(self, logits: torch.Tensor, labels: torch.Tensor, index: torch.Tensor, lam: float):
        ...
        correct = (lam * predicted.eq(labels.data).cpu().sum().float() + (1 - lam) * predicted.eq(labels[index].data).cpu().sum().float())
