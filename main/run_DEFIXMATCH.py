...
sys.path.append("./")
from configs import DEFIXMATCHConfig
...
from tasks.classification_DEFIXMATCH import Classification
...
def main():
    ...
    config = DEFIXMATCHConfig.parse_arguments()
    set_gpu(config)
...
def main_worker(local_rank: int, config: object):
    ...
    if config.data in ['cifar10', 'cifar100']:
        ...
        labeled_set = CIFAR_STRONG(data_name=config.data, dataset=datasets['l_train'], transform=train_trans)
        ...
        labeled_set = TinyImageNet_STRONG(data_name=config.data, dataset=datasets['l_train'], transform=train_trans)
        ...
        labeled_set = SVHN_STRONG(data_name=config.data, dataset=datasets['l_train'], transform=train_trans)
