...
sys.path.append("./")
from configs import PseudoLabelConfig
from datasets.cifar import CIFAR, load_CIFAR, CIFAR_WEAK_AND_RAW
from datasets.transforms import SemiAugment, TestAugment
from models import WRN, densenet121, vgg16_bn, inceptionv4
from tasks.classification_PL import Classification
...
def main():
    ...
    config = PseudoLabelConfig.parse_arguments()
    set_gpu(config)
...
def main_worker(local_rank: int, config: object):
    ...
    if config.data in ['cifar10', 'cifar100']:
        ...
        unlabeled_set = CIFAR_WEAK_AND_RAW(data_name=config.data, dataset=datasets['u_train'], transform=train_trans)
