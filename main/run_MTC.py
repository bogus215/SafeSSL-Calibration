...
sys.path.append("./")
from configs import MTcConfig
from datasets.cifar import CIFAR, load_CIFAR
from datasets.svhn import MTC_DATASET, SVHN, Selcted_DATA, load_SVHN
...
from tasks.classification_MTC import Classification
...
def main():
    ...
    config = MTcConfig.parse_arguments()
    set_gpu(config)
...
def main_worker(local_rank: int, config: object):
    ...
    domain_set = MTC_DATASET(
        data_name=config.data,
        dataset={
            key: np.concatenate([datasets["l_train"][key], datasets["u_train"][key]])
