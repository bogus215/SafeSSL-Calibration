...
from tasks.classification_SSB import Classification
...
def main():
    ...
    config = SSBConfig.parse_arguments()
    set_gpu(config)
...
def main_worker(local_rank: int, config: object):
    ...
    setattr(model,'fc_open', nn.Linear(model.output.in_features,int(model.class_num*2), bias=False))
    ...
    if config.data in ['cifar10', 'cifar100']:
        ...
        labeled_set = Selcted_DATA(dataset=datasets['l_train'], transform=train_trans, name='ssb_lb')
        unlabeled_set = Selcted_DATA(dataset=datasets['u_train'], name='ssb_ulb',transform=train_trans)
    ...
    model.run(
        ...
        lambda_ova_u=config.lambda_ova_u, 
        ova_unlabeled_threshold=config.ova_unlabeled_threshold, 
        lambda_x=config.lambda_x, 
        lambda_ova=config.lambda_ova, 
        lambda_oem=config.lambda_oem, 
