...
def main_worker(local_rank: int, config: object):
    ...
    setattr(model,'pos_head', nn.Linear(model.output.in_features,int(model.class_num+1), bias=False))
    setattr(model,'neg_head', nn.Linear(model.output.in_features,int(model.class_num+1), bias=False))
    ...
    if config.data in ['cifar10', 'cifar100']:
        ...
        labeled_set = Selcted_DATA(dataset=datasets['l_train'], transform=train_trans, name='sco_lb')
        unlabeled_set = Selcted_DATA(dataset=datasets['u_train'], name='sco_ulb',transform=train_trans)
