...
from tasks.classification_WAD import Classification
...
def main():
    ...
    config = WADConfig.parse_arguments()
    set_gpu(config)
...
def main_worker(local_rank: int, config: object):
    ...
    setattr(model,'simclr_classifier', nn.Sequential(nn.Linear(model.output.in_features, model.output.in_features),
                                                     nn.ReLU(),
                                                     nn.Linear(model.output.in_features, config.simclr_dim),
                                                     ))
