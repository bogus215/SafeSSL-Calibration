...
MISMATCH_CONFIG = {
    "imagenet": {
        "target_classes": list(range(0, 500)),
...
def load_imagenet(
    root: str, n_label_per_class: int, mismatch_ratio, random_state, logger, **kwargs
):
    ...
    images_inds_train, images_inds_valid, train_labels, val_labels = train_test_split(
    ...
    for c in tqdm(target_classes, desc="target_classes loading"):
        ...
        u_train_images.append(
            train_images[
                indices[n_label_per_class : n_label_per_class + n_unlabels_per_cls]
            ]
        )
        u_train_labels.append(
            train_labels[
                indices[n_label_per_class : n_label_per_class + n_unlabels_per_cls]
            ]
        )
