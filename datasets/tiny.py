...
def load_tiny(
    ...
    **kwargs,
    ...
    train_labels = np.array(
        [label_text_to_number[path.split("/")[-3]] for path in image_paths]
    )
    ...
    validation_dataset = {
        "images": np.array(train_images)[validation_index],
        "labels": np.array(train_labels)[validation_index],
    }
    train_dataset = {
        "images": np.array(train_images)[train_index],
    ...
    test_dataset = {"images": np.array(test_images), "labels": np.array(test_labels)}
...
if __name__ == "__main__":
    ...
    datasets, _ = load_tiny(
        root="/mnthdd/Dropbox/D/personal_study/SafeSSL+Calibration/datasets",
