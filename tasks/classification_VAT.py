...
class Classification(Task):
        ...
        with Progress(transient=True, auto_refresh=False) as pg:
            ...
            for i in range(iteration):
                with torch.cuda.amp.autocast(self.mixed_precision):
                    ...
                    unlabel_x = u_batch['x'].to(self.local_rank)
                    unlabel_y = u_batch['y'].to(self.local_rank)

                    vat_data, pseudo_logit = self.vat(torch.cat([label_x,unlabel_x],axis=0))
                    full_logits = self.predict(torch.cat([label_x, vat_data],axis=0))
                    
                    label_logit, vat_logit = full_logits[:label_y.size(0)], full_logits[label_y.size(0):]

                    label_loss = self.loss_function(label_logit, label_y.long())
                    kld_loss = self.kld(vat_logit, pseudo_logit).mean()
                    warm_up_coef = consis_coef*math.exp(-5 * (1 - min(self.trained_iteration/self.warm_up_end, 1))**2)
                    loss = label_loss + warm_up_coef*kld_loss
                ...
                result['unlabeled_top@1'][i] = TopKAccuracy(k=1)(pseudo_logit[-unlabel_y.size(0):], unlabel_y).detach()
                result['unlabeled_ece'][i] = self.get_ece(preds=pseudo_logit[-unlabel_y.size(0):].softmax(dim=1).detach().cpu().numpy(),targets=unlabel_y.cpu().numpy(),n_bins=n_bins, plot=False)[0]
                result['warm_up_coef'][i] = warm_up_coef
                result["N_used_unlabeled"][i] = unlabel_x.size(0)
    ...
    def kld(self, q_logit, p_logit):
        q = q_logit.softmax(1)
        qlogp = (q * self.logsoftmax(p_logit)).sum(1)
        qlogq = (q * self.logsoftmax(q_logit)).sum(1)
        return qlogq - qlogp

    def normalize(self, v):
        v = v / (1e-12 + self.reduce_max(v.abs(), range(1, len(v.shape))))
    ...
    def vat(self, x):
        ...
        d = self.normalize(d)
        pseudo_logit = self.predict(x).detach()

        d.requires_grad = True
        x_hat = x + self.xi * d
        y_hat = self.predict(x_hat)
        kld = self.kld(pseudo_logit, y_hat).mean()
        d = torch.autograd.grad(kld, d)[0]
        d = self.normalize(d).detach()

        x_hat = x + self.eps * d
        return x_hat, pseudo_logit