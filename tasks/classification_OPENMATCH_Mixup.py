        ...
        with Progress(transient=True, auto_refresh=False) as pg:
            ...
            for i, (data_lb, data_ulb, data_ulb_selected) in enumerate(
                ...
                with torch.cuda.amp.autocast(self.mixed_precision):
                    ...
                    input_a, input_b = (
                        torch.cat((x_lb_w_0, x_lb_w_1)),
                        torch.cat((x_lb_w_0, x_lb_w_1))[idx],
                    )
                    target_a, target_b = (
                        torch.cat((one_hot_label_y, one_hot_label_y)),
                        torch.cat((one_hot_label_y, one_hot_label_y))[idx],
                    )
                    ...
                    sup_loss = -torch.mean(
                        torch.sum(
                            nn.functional.log_softmax(logits_x_lb, dim=1)
                            * mixed_target,
                            dim=1,
                        )
                    )
                    ova_loss = ova_soft_loss_func_idx(
                        logits_open=logits_open_lb, l=l, idx=idx, label=y_lb.repeat(2)
                    )
...
def ova_soft_loss_func_idx(logits_open, l, idx, label):
    ...
    open_loss = torch.mean(
        torch.sum(-torch.log(logits_open[:, 1, :] + 1e-8) * (mixed_target), 1)
    )
    open_loss_neg = torch.mean(
