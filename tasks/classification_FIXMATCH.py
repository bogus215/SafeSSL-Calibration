...
class ImageNetClassification(Classification):
    ...
    def train(
        self, label_loader, unlabel_loader, start_fix, epoch, tau, consis_coef, n_bins
    ):
        """Training defined for a single epoch."""
        ...
        with Progress(transient=True, auto_refresh=False) as pg:
            ...
            for i, (unlabel_weak_x, unlabel_y, unlabel_strong_x) in enumerate(
                unlabel_loader
            ):
                ...
                with torch.autocast("cuda", enabled=self.mixed_precision):
                    ...
                    label_logit, unlabel_logits = (
                        full_logits[: label_x.size(0)],
                        full_logits[label_x.size(0) :],
                    )
                    unlabel_weak_logit, unlabel_strong_logit = unlabel_logits.split(
                        unlabel_weak_x.size(0)
                    )
