...
class Classification(Task):
    ...
    def train(self, label_loader, unlabel_loader, current_epoch, start_fix, tau, lambda_ova_soft, lambda_ova_cali, lambda_ova, lambda_fix, smoothing_ova, n_bins):
            ...
            for i, (data_lb, data_ulb) in enumerate(zip(label_loader, unlabel_loader)):
                with torch.cuda.amp.autocast(self.mixed_precision):
                    ...
                    if current_epoch >= start_fix:
                        ...
                        with torch.no_grad():
                            unlabel_weak_softmax = weak_logit.softmax(1)
                            unlabel_confidence, unlabel_pseudo_y = unlabel_weak_softmax.max(1)

                            ood_score = (((self.backbone.scaling_logits(weak_ova_logit, name='ova_cali_scaler')).detach()).view(len(unlabel_weak_x),2,-1).softmax(1))[torch.arange(weak_logit.size(0)),0,unlabel_weak_softmax.argmax(1)]
                    ...
                    loss = label_sup_loss + lambda_ova*label_ova_loss + lambda_ova_soft*ova_soft_loss + lambda_ova_cali*ova_cali_loss + lambda_fix*fix_loss
                ...
                if current_epoch>=start_fix:
                    if used_unlabeled_index.sum().item()!=0:
                        ...
                        result['unlabeled_ece'][i] = self.get_ece(preds=weak_logit[used_unlabeled_index].softmax(dim=1).detach().cpu().numpy(),
        ...
        with torch.no_grad():
            with Progress(transient=True, auto_refresh=False) as pg:
                ...
                for batch_idx, data in enumerate(loader):
                    ...
                    ova_logits = self.backbone.scaling_logits(self.backbone.ova_classifiers(features),name='ova_cali_scaler').view(features.size(0),2,-1)
                    outlier_score = ova_logits.softmax(1)[torch.arange(ova_logits.size(0)),0,probs.argmax(1)]
