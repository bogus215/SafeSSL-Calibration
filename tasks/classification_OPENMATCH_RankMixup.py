...
class Classification(Task):
    ...
    def run(
        ...
        **kwargs,
        ...
        self.rankmixup_loss = RankMixup_MRL(
            num_classes=self.backbone.class_num, alpha=mixup_coef
        )
    ...
    def forward_multimixup(self, x, target, num_mixup, mixup_alpha):

        if target is not None:
            target_reweighted = to_one_hot(target, self.backbone.class_num)
        ...
        for i in range(num_mixup):
            x_mix, target_reweighted_curr, lam_current = mixup_process(
                x, target_reweighted, mixup_alpha
            )
            mixup.append(x_mix)
        ...
        with Progress(transient=True, auto_refresh=False) as pg:
            ...
            for i, (data_lb, data_ulb, data_ulb_selected) in enumerate(
                ...
                with torch.cuda.amp.autocast(self.mixed_precision):
                    ...
                    x = torch.cat((x_lb_w_0, x_lb_w_1))
                    y = y_lb.repeat(2)
                    outputs = self.predict(x)
                    mixup, target_re, lam = self.forward_multimixup(
                        x, y, num_mixup=self.num_mixup, mixup_alpha=self.mixup_alpha
                    )
                    sup_loss, _, _ = self.rankmixup_loss(
                        outputs, y, mixup, target_re, lam
                    )

                    inputs = torch.cat((x, x_ulb_w_0, x_ulb_w_1))
...
class RankMixup_MNDCG(nn.Module):
    def __init__(
        self, num_classes: int = 10, alpha: float = 0.1, ignore_index: int = -100
    ):
        super().__init__()
        self.alpha = alpha
        self.ignore_index = ignore_index
    ...
    def forward(self, inputs, targets, mixup, target_re, lam):
        ...
        loss = loss_ce + self.alpha * loss_mixup  # self.alpha=0.1
...
def to_one_hot(inp, num_classes):
    ...
    y_onehot = torch.zeros(
        inp.size(0), num_classes, device=inp.device, requires_grad=False
    )
    # y_onehot.zero_()
