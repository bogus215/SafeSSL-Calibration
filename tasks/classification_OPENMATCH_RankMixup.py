...
class Classification(Task):
    ...
    def run(self,
        ...
        self.rankmixup_loss=RankMixup_MRL(num_classes=self.backbone.class_num,alpha=mixup_coef)
    ...
    def forward_multimixup(self, x, target, num_mixup, mixup_alpha):
        ...
        for i in range(num_mixup): 
            x_mix, target_reweighted_curr, lam_current = mixup_process(x, target_reweighted, mixup_alpha)
            mixup.append(x_mix)
        ...
        with Progress(transient=True, auto_refresh=False) as pg:
            ...
            for i, (data_lb, data_ulb, data_ulb_selected) in enumerate(zip(label_loader, unlabel_loader, selected_unlabel_loader)):
                with torch.cuda.amp.autocast(self.mixed_precision):
                    ...
                    mixup, target_re, lam = self.forward_multimixup(x, y, num_mixup=self.num_mixup, mixup_alpha=self.mixup_alpha)
                    sup_loss, _, _ = self.rankmixup_loss(outputs, y, mixup, target_re, lam)
