...
class Classification(Task):
    ...
    def run(self,
        ...
        for epoch in range(1, epochs + 1):
            ...
            # Train & evaluate
            train_history, train_l_iterator, train_u_iterator = self.train(train_l_iterator, train_u_iterator, iteration=save_every, threshold=threshold, consis_coef=consis_coef, n_bins=n_bins)
    ...
    def train(self, label_iterator, unlabel_iterator, threshold, consis_coef, iteration, n_bins):
        ...
        with Progress(transient=True, auto_refresh=False) as pg:
            ...
            for i in range(iteration):
                with torch.cuda.amp.autocast(self.mixed_precision):
                    ...
                    unlabel_raw_x = u_batch['raw_img'].to(self.local_rank)
                    unlabel_y = u_batch['y'].to(self.local_rank)

                    full_logits = self.predict(torch.cat([label_x, unlabel_raw_x, unlabel_weak_x],axis=0))
                    label_logit, unlabel_raw_logit, unlabel_weak_logit = full_logits.split(label_y.size(0))

                    unlabel_confidence, unlabel_pseudo_y = unlabel_raw_logit.softmax(1).max(1)

                    label_loss = self.loss_function(label_logit, label_y.long())
                    used_unlabeled_index = (unlabel_confidence>threshold)
                    if used_unlabeled_index.sum().item() == 0:
                        unlabel_loss = torch.zeros(1).to(self.local_rank)
                    else:
                        unlabel_loss = self.loss_function(unlabel_weak_logit[used_unlabeled_index], unlabel_pseudo_y[used_unlabeled_index].long().detach())
                ...
                if used_unlabeled_index.sum().item() != 0:
                    result['unlabeled_top@1'][i] = TopKAccuracy(k=1)(unlabel_raw_logit[used_unlabeled_index], unlabel_y[used_unlabeled_index]).detach()
                    result['unlabeled_ece'][i] = self.get_ece(preds=unlabel_raw_logit[used_unlabeled_index].softmax(dim=1).detach().cpu().numpy(),
