...
class Classification(Task):
    ...
    def run(self,
        ...
        for epoch in range(1, epochs + 1):
            ...
            self.logging_unlabeled_dataset(unlabeled_dataset=train_set[1],current_epoch=epoch, pi= 0.5 if epoch<start_fix else adaptive_pi, tau=tau)
            ...
            train_history, cls_wise_results = self.train(label_loader=l_loader,
                                                         ...
                                                         start_fix=start_fix,
                                                         tau=tau,
                                                         pi=0.5 if epoch<start_fix else adaptive_pi,
            ...
            if self.writer is not None:
                ...
                self.writer.add_scalar('adaptive_pi',adaptive_pi, global_step=epoch)
    ...
    def evaluate(self, data_loader, n_bins, train_n_bins, **kwargs):
        ...
        outlier_score_all = torch.cat(outlier_score_all,axis=0)
        ...
        return {k: v.mean().item() for k, v in result.items()}, train_ece_results[1], train_ece_results_ova[1], min(0.5,outlier_score_all[:,1].mean().item())
