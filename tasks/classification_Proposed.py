...
class TinyClassification(Task):
    ...
    def run(self,
        ...
        u_loader = DataLoader(train_set[1],batch_size=64*3, sampler=u_sampler,num_workers=num_workers,drop_last=False,pin_memory=False)
        ...
        with Progress(transient=True, auto_refresh=False) as pg:
            ...
            for i, (data_lb, data_ulb) in enumerate(zip(label_loader, unlabel_loader)):
                with torch.cuda.amp.autocast(self.mixed_precision):
                    ...
                    label_logit = full_logits[:label_x.size(0)]
                    weak_logit, _, strong_logit = full_logits[label_x.size(0):].chunk(3)
                    
                    ova_full_logits = self.backbone.ova_classifiers(full_features)
                    label_ova_logit = ova_full_logits[:label_x.size(0)]
                    weak_ova_logit, weak_ova_t_logit, _ = ova_full_logits[label_x.size(0):].chunk(3)
