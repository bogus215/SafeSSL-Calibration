...
class Classification(Task):
    ...
    def prepare(self,
        ...
        self.loss_function = nn.CrossEntropyLoss(label_smoothing=self.smoothingalpha)
        ...
        with Progress(transient=True, auto_refresh=False) as pg:
            ...
            for i, (data_lb, data_ulb, data_ulb_selected) in enumerate(zip(label_loader, unlabel_loader, selected_unlabel_loader)):
                with torch.cuda.amp.autocast(self.mixed_precision):
                    ...
                    ova_loss = ova_soft_loss_func(logits_open=logits_open_lb, confidence = 1-self.smoothingalpha, label=y_lb.repeat(2))
...
def ova_soft_loss_func(logits_open, confidence, label):
    ...
    label_s_sp[non_label_index.bool()] = (1-confidence)
