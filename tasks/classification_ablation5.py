                ...
                with torch.cuda.amp.autocast(self.mixed_precision):
                    ...
                    if current_epoch >= start_fix:
                        ...
                        with torch.no_grad():
                            ...
                            ood_score = ((weak_ova_logit.detach()).view(len(unlabel_weak_x),2,-1).softmax(1))[torch.arange(weak_logit.size(0)),0,unlabel_pseudo_y]
        ...
        with torch.no_grad():
            with Progress(transient=True, auto_refresh=False) as pg:
                ...
                for batch_idx, data in enumerate(loader):
                    ...
                    ova_logits = (self.backbone.ova_classifiers(features)).view(features.size(0),2,-1)
                    outlier_score = ova_logits.softmax(1)[torch.arange(probs.size(0)),0,probs.argmax(1)]
