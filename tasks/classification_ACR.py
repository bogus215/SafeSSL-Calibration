...
class Classification(Task):
        ...
        with torch.no_grad():
            with Progress(transient=True, auto_refresh=False) as pg:
                ...
                for batch_idx, data in enumerate(unlabel_loader):
                    ...
                    y = data["u_real"].cuda(self.local_rank)
    ...
    def run(
        ...
        **kwargs,
        ...
        self.adjustment_l1 = compute_adjustment_by_py(
            self.py_con, tau1, device=self.local_rank
        )
        self.adjustment_l12 = compute_adjustment_by_py(
            self.py_con, tau12, device=self.local_rank
        )
        self.adjustment_l2 = compute_adjustment_by_py(
            self.py_con, tau2, device=self.local_rank
        )
        self.taumin = 0
        self.taumax = tau1
        self.ema_u = ema_u
        ...
        for epoch in range(1, epochs + 1):
            ...
            self.log_unlabeled_data(unlabel_loader=unlabel_loader_for_log, current_epoch=epoch)

            if epoch > self.est_epoch:
                count_KL = count_KL / len(l_loader)
                KL_softmax = (torch.exp(count_KL[0])) / (
                    torch.exp(count_KL[0])
                    + torch.exp(count_KL[1])
                    + torch.exp(count_KL[2])
                )
                tau = self.taumin + (self.taumax - self.taumin) * KL_softmax
                if math.isnan(tau) == False:
                    self.adjustment_l1 = compute_adjustment_by_py(
                        self.py_con, tau, device=self.local_rank
                    )
            count_KL = torch.zeros(3).to(self.local_rank)

            train_history, count_KL = self.train(
    ...
    def train(
        self, label_loader, unlabel_loader, current_epoch, start_fix, n_bins, count_KL
    ):
        ...
        with Progress(transient=True, auto_refresh=False) as pg:
            ...
            for i, (data_lb, data_ulb) in enumerate(zip(label_loader, unlabel_loader)):
                ...
                with torch.cuda.amp.autocast(self.mixed_precision):
                    ...
                    logits_u_w_b, logits_u_s_b, logits_u_s1_b = logits_b[
                        batch_size:
                    ].chunk(3)
                    del logits_b
                    Lx_b = F.cross_entropy(
                        logits_x_b + self.adjustment_l2, targets_x, reduction="mean"
                    )

                    pseudo_label = torch.softmax(
                        (logits_u_w.detach() - self.adjustment_l1) / self.T, dim=-1
                    )
                    pseudo_label_h2 = torch.softmax(
                        (logits_u_w.detach() - self.adjustment_l12) / self.T, dim=-1
                    )
                    pseudo_label_b = torch.softmax(
                        logits_u_w_b.detach() / self.T, dim=-1
                    )
                    ...
                    mask_twice_ss_b_h2 = torch.cat(
                        [mask_ss_b_h2, mask_ss_b_h2], dim=0
                    ).cuda()
                    mask_twice_ss_t = torch.cat([mask_ss_t, mask_ss_t], dim=0).cuda()

                    logits_u_s_twice = torch.cat(
                        [logits_u_s, logits_u_s1], dim=0
                    ).cuda()
                    targets_u_twice = torch.cat([targets_u, targets_u], dim=0).cuda()
                    targets_u_h2_twice = torch.cat(
                        [targets_u_h2, targets_u_h2], dim=0
                    ).cuda()

                    logits_u_s_b_twice = torch.cat(
                        [logits_u_s_b, logits_u_s1_b], dim=0
                    ).cuda()
                    ...
                    if current_epoch > self.est_epoch:
                        ...
                        if now_mask.sum() > 0:
                            ...
                            self.u_py = (
                                self.ema_u * self.u_py + (1 - self.ema_u) * now_mask
                            )
                            KL_con = 0.5 * KL_div(
                                self.py_con.log(), self.u_py
                            ) + 0.5 * KL_div(self.u_py.log(), self.py_con)
                            KL_uni = 0.5 * KL_div(
                                self.py_uni.log(), self.u_py
                            ) + 0.5 * KL_div(self.u_py.log(), self.py_uni)
                            KL_rev = 0.5 * KL_div(
                                self.py_rev.log(), self.u_py
                            ) + 0.5 * KL_div(self.u_py.log(), self.py_rev)
                    ...
                    Lu = (
                        F.cross_entropy(
                            logits_u_s_twice, targets_u_twice, reduction="none"
                        )
                        * mask_twice_ss_t
                    ).mean()
                    Lu_b = (
                ...
                result["ece"][i] = self.get_ece(
                    preds=logits_x.softmax(dim=1).detach().cpu().numpy(),
