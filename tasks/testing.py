...
class Testing(Task):
    ...
    def prepare(
        ...
        **kwargs,
        ...
        self.local_rank = (
            "cpu" if local_rank < 0 else local_rank
        ...
        try:
            ...
            for old_key, new_key in zip(
                ...
                ["output.linear.weight", "output.linear.bias"],
            ):
    ...
    def run(
        ...
        **kwargs,
        ...
        if for_what == "Proposed":
            ...
            eval_history = self.safestudent_evaluate(
                open_test_loader,
                mismatch_ratio=mismatch_ratio,
                safe_student_T=safe_student_T,
            )
        ...
        elif for_what == "Ablation4":
            eval_history = self.ablation4_evaluate(open_test_loader)
        elif for_what == "Ablation5":
    ...
    def mtc_evaluate(self, data_loader, **kwargs):
        ...
        result["AUROC"][0] = roc_auc_score(
            y_true=ood_label.cpu(), y_score=(1 - domain_score).cpu()
        )
        result["In distribution over conf 0.95: ECE"][0] = self.get_ece(
            preds=logits[
        ...
        result["In distribution under ood score 0.5: ECE"][0] = self.get_ece(
    ...
    @torch.no_grad()
    def safestudent_evaluate(
        self, data_loader, mismatch_ratio, safe_student_T, **kwargs
    ):
        """Evaluation defined for a single epoch."""
        ...
        with Progress(transient=True, auto_refresh=False) as pg:
            ...
            for i, batch in enumerate(data_loader):
                ...
                score = self.ed(logits=logit, t=safe_student_T)
        ...
        labels, logits, scores = (
            torch.cat(labels, axis=0),
            torch.cat(logits, axis=0),
            torch.cat(scores, axis=0),
        )
        scores = (scores - scores.min()) / (scores.max() - scores.min())

        threshold = threshold_otsu(scores.cpu().numpy())

        in_label = torch.where(labels < logits.size(1), 1, 0)
        in_pred = (scores > threshold).cpu()
        ood_pred = (scores <= threshold).cpu()
        ood_label = torch.where(labels >= logits.size(1), 1, 0)
        ...
        result["AUROC"][0] = roc_auc_score(y_true=ood_label.cpu(), y_score=1 - scores)
        result["In distribution over conf 0.95: ECE"][0] = self.get_ece(
            preds=logits[
                (labels < logits.size(1)) & (logits.softmax(1).max(1)[0] >= 0.95)
            ]
    ...
    def iomatch_evaluate(self, data_loader, **kwargs):
        ...
        with Progress(transient=True, auto_refresh=False) as pg:
            ...
            for i, batch in enumerate(data_loader):
                ...
                logits_open = self.backbone.openset_classifier(
                    feat_proj
                )  # (k+1)-way logits
                logits_mb_x_ulb_w = self.backbone.mb_classifiers(
                    feat_proj
                )  # shape: [bsz, 2K]
    ...
    def fixmatch_evaluate(self, data_loader, **kwargs):
        ...
        result["AUROC"][0] = roc_auc_score(
            y_true=ood_label.cpu(), y_score=(1 - logits.softmax(1).max(1)[0]).cpu()
        )
        result["F1-IN"][0] = f1_score(y_true=in_label, y_pred=in_pred)
        result["F1-OOD"][0] = f1_score(y_true=ood_label, y_pred=ood_pred)
        result["In distribution over conf 0.95: ECE"] = self.get_ece(
    ...
    def openmatch_evaluate(self, data_loader, ova_pi, **kwargs):
        ...
        with Progress(transient=True, auto_refresh=False) as pg:
            ...
            for i, batch in enumerate(data_loader):
                ...
                tmp_range = (
                    torch.arange(0, logits_open.size(0)).long().cuda(self.local_rank)
                )
        ...
        labels, logits, out_scores, in_scores, ova_logits = (
            ...
            torch.cat(logits, axis=0),
            torch.cat(out_scores, axis=0),
            torch.cat(in_scores, axis=0),
            torch.cat(ova_logits, axis=0),
        )
        ...
        result["top@1-ova"][0] = TopKAccuracy(k=1)(
            ova_logits.softmax(dim=1)[labels < logits.size(1), 1, :],
        ...
        result["ECE-ova"][0] = self.get_ece(
            preds=ova_logits.softmax(dim=1)[labels < logits.size(1), 1, :].numpy(),
            targets=labels[labels < logits.size(1)].numpy(),
            plot_title="_all_ova",
        )

        ovr_confidence, ovr_results = torch.cat(
            [out_scores.unsqueeze(1), in_scores.unsqueeze(1)], axis=1
        ).max(1)

        result["AUROC"][0] = roc_auc_score(
            y_true=ood_label.cpu()[ovr_confidence >= ova_pi],
            y_score=out_scores.cpu()[ovr_confidence >= ova_pi],
        )
        result["F1-IN"][0] = f1_score(
            y_true=in_label[ovr_confidence >= ova_pi],
            y_pred=in_pred[ovr_confidence >= ova_pi],
        )
        result["F1-OOD"][0] = f1_score(
            y_true=ood_label[ovr_confidence >= ova_pi],
            y_pred=ood_pred[ovr_confidence >= ova_pi],
        )
        result["Accuracy"][0] = accuracy_score(
            y_true=in_label[ovr_confidence >= ova_pi],
            y_pred=in_pred[ovr_confidence >= ova_pi],
        )

        result["IN-SEEN-DETECTION-ECE"][0] = self.seen_unseen_detection_get_ece(
            predicted_label=in_pred[labels < logits.size(1)].numpy(),
            confidences=ovr_confidence[labels < logits.size(1)].numpy(),
            targets=in_label[labels < logits.size(1)].numpy(),
            title="IN",
            n_bins=15,
        )
        result["OOD-SEEN-DETECTION-ECE"][0] = self.seen_unseen_detection_get_ece(
            predicted_label=in_pred[labels >= logits.size(1)].numpy(),
            confidences=ovr_confidence[labels >= logits.size(1)].numpy(),
            targets=in_label[labels >= logits.size(1)].numpy(),
            title="OOD",
        ...
        result["ALL-SEEN-DETECTION-ECE"][0] = self.seen_unseen_detection_get_ece(
            predicted_label=in_pred.numpy(),
            confidences=ovr_confidence.numpy(),
            ...
            pickle.dump(
                {
                    ...
                    "confidences": ovr_confidence.numpy(),
                    "targets": in_label.numpy(),
                    "selected": ((ovr_confidence >= ova_pi) & (labels < logits.size(1)))
                    .sum()
                    .item(),
                },
                f,
    ...
    def ablation3_evaluate(self, data_loader, **kwargs):
        ...
        with Progress(transient=True, auto_refresh=False) as pg:
            ...
            for i, batch in enumerate(data_loader):
                ...
                ova_logit = (self.backbone.ova_classifiers(features)).view(
                    features.size(0), 2, -1
                )
        ...
        result["AUROC"][0] = roc_auc_score(
            y_true=ood_label.cpu(), y_score=ova_scores[:, 0]
        )
        ...
        result["IN-SEEN-DETECTION-ECE"][0] = self.seen_unseen_detection_get_ece(
            predicted_label=in_pred[labels < logits.size(1)].numpy(),
            confidences=ova_scores.max(1)[0][labels < logits.size(1)].numpy(),
            targets=in_label[labels < logits.size(1)].numpy(),
            title="IN",
            n_bins=15,
        )
    ...
    def ablation1_evaluate(self, data_loader, **kwargs):
        ...
        with Progress(transient=True, auto_refresh=False) as pg:
            ...
            for i, batch in enumerate(data_loader):
                ...
                ova_logit = self.backbone.scaling_logits(
    ...
    def proposed_evaluate(self, data_loader, ova_pi, **kwargs):
        ...
        result["AUROC"][0] = roc_auc_score(
            y_true=ood_label[ova_scores.max(1)[0] >= ova_pi].cpu(),
            y_score=ova_scores[ova_scores.max(1)[0] >= ova_pi, 0],
        )

        result["F1-IN"][0] = f1_score(
            y_true=in_label[ova_scores.max(1)[0] >= ova_pi],
            y_pred=in_pred[ova_scores.max(1)[0] >= ova_pi],
        )
        result["F1-OOD"][0] = f1_score(
            y_true=ood_label[ova_scores.max(1)[0] >= ova_pi],
            y_pred=ood_pred[ova_scores.max(1)[0] >= ova_pi],
        )
            ...
            pickle.dump(
                {
                    ...
                    "targets": in_label.numpy(),
                    "selected": (
                        (ova_scores.max(1)[0] >= ova_pi) & (labels < logits.size(1))
                    )
                    .sum()
                    .item(),
                },
    ...
    def get_ece(self, preds: np.array, targets: np.array, n_bins: int = 15, **kwargs):
        ...
        plt.bar(
            np.array(x_ticks)[np.array(x_ticks) != None],
            np.array(confs_ticks)[np.array(x_ticks) != None],
            label="Confidence",
            alpha=0.7,
            ecolor="blue",
            width=bin_lowers[1],
        ...
        plt.bar(
            np.array(x_ticks)[np.array(x_ticks) != None],
            np.array(acc_ticks)[np.array(x_ticks) != None],
            label="Accuracy",
            alpha=0.7,
            ecolor="pink",
            width=bin_lowers[1],
        ...
        plt.legend()
        plt.savefig(
            os.path.join(self.ckpt_dir, f"Reliability_diagrams{plot_title}.png")
        )
        plt.close("all")

        plt.bar(
            np.array(x_ticks)[np.array(x_ticks) != None],
            np.array(y_ticks_second_ticks)[np.array(x_ticks) != None],
            ecolor="blue",
            width=bin_lowers[1],
            edgecolor="black",
        )
        ...
        plt.axvline(
            x=confidences.mean(), label="Avg. confidence", color="blue", linestyle="--"
        )
        plt.legend()
        plt.xlabel("Confidence")
        ...
        plt.ylim(0, 1)
        plt.savefig(
            os.path.join(self.ckpt_dir, f"Confidence_histogram{plot_title}.png")
        )
        plt.close("all")

        return ece

    def seen_unseen_detection_get_ece(
        self,
    ...
    @torch.no_grad()
    def ablation4_evaluate(self, data_loader, **kwargs):
        """Evaluation defined for a single epoch."""
        ...
        with Progress(transient=True, auto_refresh=False) as pg:
            ...
            for i, batch in enumerate(data_loader):
                ...
                ova_score = ova_logit.softmax(1)[
                    torch.arange(ova_logit.size(0)), :, probs.argmax(1)
                ]
    ...
    @torch.no_grad()
    def ablation5_evaluate(self, data_loader, **kwargs):
        """Evaluation defined for a single epoch."""
...
class ImageNetTesting(Testing):
    ...
    def proposed_evaluate(self, data_loader, ova_pi, **kwargs):
        ...
        result["top@5"][0] = TopKAccuracy(k=5)(
            logits[labels < logits.size(1)], labels[labels < logits.size(1)]
        )
        result["top@1-ova"][0] = TopKAccuracy(k=1)(
            ova_logits.softmax(dim=1)[labels < logits.size(1), 1, :],
