...
class Classification(Task):
    ...
    def run(self,
        ...
        d_sampler = DistributedSampler(dataset=domain_set, num_replicas=1, rank=self.local_rank, num_samples=num_samples)
        domain_loader = DataLoader(domain_set,batch_size=self.batch_size//2, sampler=d_sampler,num_workers=num_workers,drop_last=False,pin_memory=False)
        ...
        for epoch in range(epochs//10 + 1, epochs + 1):
            ...
            train_history = self.train(label_loader=l_loader, selected_unlabel_loader=selected_u_loader, domain_loader=domain_loader, T=T, alpha=alpha, lambda_u=lambda_u)
            eval_history = self.evaluate(eval_loader, n_bins)
        ...
        with Progress(transient=True, auto_refresh=False) as pg:
            ...
            for i, (d_data, l_data) in enumerate(zip(d_loader,l_loader)):
                with torch.cuda.amp.autocast(self.mixed_precision):
                    ...
                    domain_logit = self.backbone.domain_classifier(feat)
                    domain_loss = nn.functional.binary_cross_entropy_with_logits(domain_logit.squeeze(), torch.cat((soft_label,torch.ones_like(soft_label))))
                ...
                result['domain_top@1'][i] = ((target<self.backbone.class_num) == (domain_logit[:weak_img.size(0)].sigmoid()>threshold).squeeze()).sum() / weak_img.size(0)
                result['top@1'][i] = TopKAccuracy(k=1)(logit[-x_lb.size(0):], y_lb).detach()
                ...
                pred_soft_labels[idx] = domain_logit[:weak_img.size(0)].sigmoid().squeeze().detach().float()

        d_loader.dataset.label_update(pred_soft_labels)

        return {k: v.mean().item() for k, v in result.items()}, threshold_otsu(pred_soft_labels[len(l_loader.dataset):].cpu().numpy())

    def train(self, label_loader, selected_unlabel_loader, domain_loader, T,alpha, lambda_u):
        ...
        pred_soft_labels = torch.zeros(len(domain_loader.dataset), device=self.local_rank)
        with Progress(transient=True, auto_refresh=False) as pg:

            if self.local_rank == 0:
                task = pg.add_task(f"[bold red] Training...", total=iteration)

            for i, (data_lb, data_ulb_selected, data_domain) in enumerate(zip(label_loader, selected_unlabel_loader, domain_loader)):
                with torch.cuda.amp.autocast(self.mixed_precision):
                    ...
                    one_hot_label_y = torch.nn.functional.one_hot(label_y,self.backbone.class_num)

                    with torch.no_grad():
                        # compute guessed labels of unlabel samples
                        outputs_u, outputs_u2 = logits[-2*unlabel_weak_x.size(0):].chunk(2)
                    ...
                    L_x, L_u = mixmatch_loss(logits_x, mixed_target[:label_x.size(0)], logits_u, mixed_target[-label_x.size(0):])
                ...
                result['top@1'][i] = TopKAccuracy(k=1)(logits[:label_x.size(0)], label_y).detach()

                pred_soft_labels[d_indexs] = domain_logit[label_x.size(0):].sigmoid().squeeze().detach().float()
...
def mixmatch_loss(outputs_x, targets_x, outputs_u, targets_u):
    ...
    Lx = -torch.mean(torch.sum(nn.functional.log_softmax(outputs_x, dim=1) * targets_x, dim=1))
