    ...
    def train(self, label_loader, unlabel_loader, current_epoch, start_fix, n_bins, ood_threshold):
        ...
        with Progress(transient=True, auto_refresh=False) as pg:
            ...
            for i, (data_lb, data_ulb) in enumerate(zip(label_loader, unlabel_loader)):
                ...
                if i % self.threshold_update_freq == 0 and i > 0 and current_epoch >= start_fix:
                    max_len = sum([len(self.all_sample_scores[i]) for i in range(self.backbone.class_num)])
                ...
                with torch.cuda.amp.autocast(self.mixed_precision):
                    ...
                    if self.selected_ood_count < b_size:
                        ...
                        ood_scores = torch.tensor(list(self.selected_ood_scores))[indices].to(self.local_rank)
                        ood_label = (torch.ones(b_size) * self.backbone.class_num).to(self.local_rank).long()
                        ...
                        logits_open_w, logits_open_s, logits_close_w, logits_close_s = logits_p[b_size+b_size:].chunk(4)
                    ...
                    max_probs_open, _ = torch.max(pseudo_label_open[:, :self.backbone.class_num], dim=-1)
                    ...
                    for prob, img, ulab in zip(max_probs_open[indices], inputs_all_w[indices.cpu()], targets_u_eval[indices.cpu()]):
                    ...
                    mask_pos = max_probs_open.ge(self.threshold) & (targets_u_all_open < self.backbone.class_num)
                    mask_pos = mask_pos | ((max_probs_open.ge(ood_threshold)) & (targets_u_all_open == self.backbone.class_num))
                    L_unsup_open = (
                            F.cross_entropy(
                ...
                result['top@1'][i] = TopKAccuracy(k=1)(logits_id_lb[:,:self.backbone.class_num], targets_x).detach()
                result['ece'][i] = self.get_ece(preds=logits_id_lb[:,:self.backbone.class_num].softmax(dim=1).detach().cpu().numpy(), targets=targets_x.cpu().numpy(), n_bins=n_bins, plot=False)[0]
...
class TinyClassification(Task):
    ...
    def prepare(self,
        ...
        self.scheduler = get_cosine_schedule_with_warmup(self.optimizer, 0, self.iterations)
