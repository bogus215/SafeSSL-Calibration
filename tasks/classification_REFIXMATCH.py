        ...
        with Progress(transient=True, auto_refresh=False) as pg:
            ...
            for i in range(iteration):
                with torch.cuda.amp.autocast(self.mixed_precision):
                    ...
                    unsup_loss_refix = nn.functional.kl_div(nn.functional.log_softmax(logits_x_ulb_s / self.pseudolabelhook.T, dim=-1), probs_x_ulb_w, reduction='none')
                    unsup_loss_refix = torch.sum(unsup_loss_refix * (1.0 - mask.float()).unsqueeze(dim=-1).repeat(1, logits_x_ulb_s.size(-1)), dim=1).mean()

                    warm_up_coef = consis_coef*math.exp(-5 * (1 - min(self.trained_iteration/self.warm_up_end, 1))**2)
                    loss = sup_loss + warm_up_coef*(unsup_loss+unsup_loss_refix)
                ...
                unique, counts = np.unique(unlabel_y[mask].cpu().numpy(), return_counts = True)
